...
namespace SpectatorView
{
    ...
    public class SV_RemotePlayerManager : SpectatorView.SV_Singleton<SV_RemotePlayerManager>
    {
        ...
        void UpdateHeadTransform(NetworkInMessage msg)
        {
         ...
         //   if(SpectatorView.HolographicCameraManager.Instance.tppcUser != null)
         //       Debug.Log("ID : " + SpectatorView.HolographicCameraManager.Instance.tppcUser.GetID() + ", userID" + userID);
            ...
            if (headInfo.HeadObject != null)
            {
                    ...
                    SpectatorView.HolographicCameraManager.Instance.tppcUser.GetID() == userID)
                {
                    ...
                    cP = (prevPos + headPos) * 0.5f;
                    cR = new Quaternion((prevRot.x + headRot.x) * 0.5f, (prevRot.y + headRot.y) * 0.5f, (prevRot.z + headRot.z) * 0.5f, (prevRot.w + headRot.w) * 0.5f);

                    // SetHologramPose(headRot.x, headRot.y, headRot.z, headRot.w, headPos.x, headPos.y, headPos.z);
                    SetHologramPose(cR.x, cR.y, cR.z, cR.w, cP.x, cP.y, cP.z);
                    
                    prevPos = headPos;
                    prevRot = headRot;

                //    Debug.Log("if Pos : (" + cP.x.ToString() + ", " + cP.y.ToString() + ", " + cP.z.ToString() + ")");
                //    Debug.Log("Rot : (" + headRot.eulerAngles.ToString() + ")");
                }
                else
#endif
                {
                 ...
                 //   Debug.Log("else : Pos : " + headPos + ", rot : " + headRot);
                }
